...
BOOST_AUTO_TEST_SUITE(CKKSToTFHETest)
  ...
  RC_BOOST_PROP(toLv3TRLWE,
    ...
    // Generate Key
    seal::KeyGenerator keygen(context);
    static const auto &secretKey = keygen.secret_key();
    ...
    // Assert the result
    if (value > 0) {
      std::cout << "R: expected 0, actual " << negative << std::endl;
      RC_ASSERT(!negative);
    } else if (value < 0) {
      std::cout << "R: expected 1, actual " << negative << std::endl;
      RC_ASSERT(negative);
    }
  }
  ...
  RC_BOOST_PROP(toLv3TLWE,
    ...
    // Assert the result
    if (value > 0) {
      std::cout << "3: expected 0, actual " << negative << std::endl;
      RC_ASSERT(!negative);
    } else if (value < 0) {
      std::cout << "3: expected 1, actual " << negative << std::endl;
      RC_ASSERT(negative);
    }
  }
  ...
  RC_BOOST_PROP(toLv1TLWE,
    ...
    // Generate Key
    static seal::KeyGenerator keygen(context);
    static const auto &secretKey = keygen.secret_key();
    ...
    static const ArithHomFA::BootstrappingKey bootKey(skey, lvl3Key, lvlhalfkey);
    ArithHomFA::Lvl3ToLvl1 ThreeToOne(bootKey);
    converter.initializeConverter(bootKey);

    // Convert the ciphertext for TFHEpp (lvl1)
    TFHEpp::TLWE<TFHEpp::lvl1param> tlwe, tlweM;
    TFHEpp::TLWE<TFHEpp::lvl3param> tlweThree;
    converter.toLv1TLWE(cipher, tlwe);
    converter.toLv3TLWE(cipher, tlweThree);
    ...
    // Assert the result
    if (value > 0) {
      std::cout << "expected 0, actual " << negative << " " << negativeM << " " << negativeThree << std::endl;
      RC_ASSERT(!negative);
    } else if (value < 0) {
      std::cout << "expected 1, actual " << negative << " " << negativeM << " " << negativeThree << std::endl;
      RC_ASSERT(negative);
    }
  }
