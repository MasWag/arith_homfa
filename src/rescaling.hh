...
namespace ArithHomFA {
    ...
    public:
        explicit Rescaling(const seal::SEALContext::ContextData &context_data) {
            ...
            decryption_modulus = seal::util::allocate_zero_uint(original_modulus_size + 1,
        ...
        std::uint64_t rescale(const std::uint64_t *const coefficient) const {
            ...
            // 1. Check the sign
            bool isNegative = seal::util::is_greater_than_or_equal_uint(
                    coefficient, upper_half_threshold, original_modulus_size);
            ...
            assert(seal::util::is_less_than_or_equal_uint(numerator.get() + 1, upper_half_threshold,
                                                          original_modulus_size));

            // 3. Divide by the law
            seal::util::divide_uint_inplace(numerator.get(), decryption_modulus.get(),
                                            original_modulus_size + 1, quotient.get(),
                                            seal::MemoryManager::GetPool());
