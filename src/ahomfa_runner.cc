...
namespace {
    ...
    void register_pointwise(CLI::App &app, Args &args) {
        CLI::App *pointwise = app.add_subcommand("pointwise",
                                                 "Evaluate the given signal point-wise (for debugging)");
        ...
        pointwise->add_option_function("-f,--file", configCallback, "The configuration file of SEAL")->required();

        std::function<void(const std::string &)> callback = [&args](const std::string &path) {
            args.input = new std::ifstream(path);
        };
        pointwise->add_option_function("-i,--input", callback, "The file to load the input");

        std::function<void(const std::string &)> output_callback = [&args](const std::string &path) {
            args.output = new std::ofstream(path);
        };
        pointwise->add_option_function("-o,--output", output_callback, "The file to write the result");
        ...
        pointwise->add_option_function("-s,--seal-key", keyCallback, "The relinearization key of SEAL");
    }

    void do_pointwise(const ArithHomFA::SealConfig &config, const seal::RelinKeys &relinKeys, std::istream& istream, std::ostream& ostream) {
