...
{
    for (size_t i = 0; i < (Lvl1::k+1) * Lvl1::l; i++)
        for (size_t j = 0; j < Lvl1::k+1; j++)
...
{
    TLWELvl0 ret = {};
    ret[Lvl0::n] = -TFHEpp::lvl0param::μ;  // -1/8
    return ret;
}
...
{
    TLWELvl1 ret = {};
    ret[Lvl1::n] = -TFHEpp::lvl1param::μ;  // -1/8
    return ret;
}
...
TRLWELvl1 trivial_TRLWELvl1_minus_1over8()
{
    ...
    ret[TFHEpp::lvl1param::k][0] = -TFHEpp::lvl1param::μ;  // -1/8
    return ret;
}
...
{
    TRLWELvl1 ret = trivial_TRLWELvl1_zero();
    ret[1][0] = 4*TFHEpp::lvl1param::μ;  // 1/2
    return ret;
}
...
{
    for(size_t i = 0; i < Lvl1::k+1; i++)
        PolyLvl1_mult_X_k(out[i], src[i], k);
}
                                  ...
                                  const EvalKey& ek)
{
    ...
    std::get<TFHEpp::lvl0param::k*Lvl0::n>(src) -= 2*TFHEpp::lvl0param::μ;  // 1/4
    // Bootstrapping without changing plaintext space
    BlindRotate<lvl01param>(out, src, ek.getbkfft<lvl01param>(),
                            μpolygen<Lvl1, 2*TFHEpp::lvl1param::μ /* 1/4 */>());
    // Convert {-1/4, 1/4} to {0, 1/2}
    out[TFHEpp::lvl1param::k][0] += 2*TFHEpp::lvl1param::μ;  // 1/4
}
                                     ...
                                     const EvalKey& ek)
{
    ...
    // Convert {0, 1/2} to {-1/4, 1/4}
    src[Lvl0::n] -= 2*TFHEpp::lvl0param::μ;  // 1/4
    // Bootstrapping and convert {-1/4, 1/4} to {-1/8, 1/8}
    BlindRotate<lvl01param>(out, src, ek.getbkfft<lvl01param>(),
                            μpolygen<Lvl1, TFHEpp::lvl1param::μ /* 1/8 */>());
}
