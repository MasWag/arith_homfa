...
This directory hosts everything needed to build and publish the user and developer manuals. It only covers documentation tooling—see the repository root `README.md` for the project overview, requirements, and usage guide.
...
- When adding material from `README.md`, place the full explanation inside `doc/docs/` and keep only a pointer in the README to avoid divergence.
- Maintain section anchors (`## 3.4 …`) so hyperlinks from `mkdocs.yml` remain valid.
- Run `mkdocs serve` locally before pushing structural changes to catch broken links or formatting regressions.
